...
(defn process-tweet 
  ...
  [{:keys [links] :as tw-list}
   ...
   (if-let [link (seq (for [my-link (seq links)  
     ...
     (update-in tw-list [:links] conj
                 (map->Tw-link {:url (:expanded_url (first (:urls entities)))
...
(defn process-list-tweets 
  ...
  (let [tweets (:body (twitter.api.restful/lists-statuses 
    ...
    (reduce #(process-tweet %1 %2) 
            (get-latest-tweet-id tw-list tweets) tweets)))
...
(defn update-tw-links 
  ...
  [tw-lists] ; i hate this form/pattern need to clean that up
  (if (nil? tw-lists)
    (read-list-tweets (get-new-tw-lists))
...
(defn -main
  ([cfg] (update-tw-links (read-tw-cfg (new-tw-list) cfg)))
